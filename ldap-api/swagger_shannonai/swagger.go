...
import (
	...
	//"github.com/swagger_shannonai/swaggerFiles"
	"ldap-api/swagger_shannonai/swaggerFiles"
...
// WrapHandler wraps `http.Handler` into `beego`.
...
func (c *SwaggerController) Get() {
	...
	c.TplName = "swagger_index.tpl"
	t := template.New("swagger_index.tpl")
	index, _ := t.Parse(swagger_index_templ)
	type pro struct {
		Host string
	}
	var re = regexp.MustCompile(`(.*)(index\.html|doc\.json|favicon-16x16\.png|favicon-32x32\.png|/oauth2-redirect\.html|swagger-ui\.css|swagger-ui\.css\.map|swagger-ui\.js|swagger-ui\.js\.map|swagger-ui-bundle\.js|swagger-ui-bundle\.js\.map|swagger-ui-standalone-preset\.js|swagger-ui-standalone-preset\.js\.map)[\?|.]*`)
	var matches []string
	if matches = re.FindStringSubmatch(c.Ctx.Request.RequestURI); len(matches) != 3 {
		c.Ctx.Abort(404, "404 page not found")
		return
	}
	path := matches[2]
	prefix := matches[1]
	...
	case "index.html":
		...
		index.Execute(c.Ctx.ResponseWriter, s)
	case "doc.json":
		doc, _ := swag.ReadDoc()
		c.Ctx.ResponseWriter.Write([]byte(doc))
		return
	default:
